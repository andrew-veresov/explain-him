---
tags: [explain-him, home]
---

# Explain Him

## Start here

- [[README|Overview and quick start]]
- [[WEBMCP_CHALLENGE|WebMCP Challenge judge guide]]
- [[knowledge/00-overview|How Explain Him works]]
- [[knowledge/01-originator-flow|What the Originator does]]
- [[knowledge/02-user-agent-flow|How the personal agent works]]

## Meaning and grounding

- [[knowledge/03-grounding-and-status|Grounding, provenance, and statuses]]
- [[knowledge/04-question-loop|New-question loop]]
- [[resolutions/README|Accepted resolutions]]

## Adaptive presentation and WebMCP

- [[knowledge/06-browser-local-workspace|Browser-local workspace]]
- [[knowledge/07-presentation-capabilities|Presentation Capabilities]]
- [[resolutions/2026-08-30-webmcp-challenge-surface|Current WebMCP boundary]]
- [[resolutions/2026-08-29-presentation-capabilities|Presentation Capability boundary]]
- [Open the executable page](index.html)

## Implementation

- [[AGENTS|Instructions for agents]]
- [[skills/explain-him/SKILL|Repository skill]]
- [[CONTRIBUTING|Contribution rules]]
- [[SECURITY|Security]]
