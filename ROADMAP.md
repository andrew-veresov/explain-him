...
The [Product Contract](PRODUCT-CONTRACT.md) is the source of truth for product intent. This roadmap records the Protocol v4 implementation and the remaining production acceptance path in ChatGPT Desktop built-in browser Site Tools. It is not evidence that an account or model already has Site Tools or the experimental native-skill API.

## Current baseline

- A static GitHub Pages explanation with immutable authored content.
- A reversible browser-local Personalized layer.
- Exactly two Protocol v4 WebMCP tools: `get_explain_him_context` and `explain_tool`.
- Immutable pinned A8 grounding and presentation skills.
...
## Protocol v4 context and display

1. Every explanation request begins with `get_explain_him_context` and inspects Original plus Personalized UI.
...
## A8 progressive native skill

1. Current tools register independently and remain exactly `get_explain_him_context` and `explain_tool`.
...
3. One generated inline skill combines the ordered A8 grounding and presentation instructions with provenance, structured context, annotations, and both tool names.
