...
2. If Site Tools are available, the agent calls `get_explain_him_answer` to discover the public repository, both repository-scoped skills, the typed-block schema, and stable authored targets.
...
User ↔ browser/personal agent
              │
              ├── get_explain_him_answer → load both skills
