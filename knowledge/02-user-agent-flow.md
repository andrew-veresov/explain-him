...
2. If Site Tools are available, the agent inspects the visible Original/Personalized UI through the host and calls `explain_tool` directly for the requested explanation action.
...
User ↔ browser/personal agent
              │
              ├── inspect visible page and repository guidance
              ├── answer in agent chat
              └── explain_tool → inspect state + focus or add / replace / update / remove
                           │
                           ▼
