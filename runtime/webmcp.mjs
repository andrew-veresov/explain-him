...
import { EXPLAIN_HIM_NATIVE_SKILL, EXPLAIN_HIM_NATIVE_SKILL_DIGEST } from './generated/explain-him-native-skill.mjs';
...
const ANSWER_WORKFLOW = Object.freeze({
  ...
  steps: Object.freeze([
    ...
    Object.freeze({ order: 2, id: 'load-pinned-skills', action: 'Use the page-issued native-inline proof and verified inline composite when available; otherwise fetch every skill in skillLoadOrder from its immutable raw URL and verify its commit and SHA-256.' }),
    ...
    Object.freeze({ order: 7, id: 'apply-required-adaptation', action: 'When agentPolicy requires a page change, call apply_explanation in the same turn with the complete A7 Protocol v3 handshake and page-issued skill-delivery proof.' }),
...
const AGENT_POLICY = Object.freeze({
  ...
  skillLoading: { required: true, loadOrder: LOAD_ORDER, deliveryModes: ['native-inline', 'pinned-remote-fallback'], nativeRegistrationDoesNotProveSemanticReading: true },
...
function verifyDeliveryProof(value, expected) { const input = strictObject(value, 'handshake.skillDeliveryProof', ['mode', 'deliveryId', 'registrationId', 'compositeSha256', 'sourceCommit', 'sourceProof']); if (!['native-inline', 'pinned-remote-fallback'].includes(input.mode)) throw new TypeError('handshake.skillDeliveryProof.mode is invalid'); required(input.deliveryId, 'handshake.skillDeliveryProof.deliveryId', 160); if (input.registrationId !== null) required(input.registrationId, 'handshake.skillDeliveryProof.registrationId', 160); required(input.compositeSha256, 'handshake.skillDeliveryProof.compositeSha256', 64); required(input.sourceCommit, 'handshake.skillDeliveryProof.sourceCommit', 40); verifyProof(input.sourceProof); if (stable(input) !== stable(expected)) throw new TypeError('handshake.skillDeliveryProof is stale, forged, or does not belong to this page activation'); return clone(input); }
function handshake(input, workspace, current) { if (!input || typeof input !== 'object' || Array.isArray(input)) throw new TypeError('input must be an object'); const allowed = new Set(HANDSHAKE_FIELDS); for (const key of Object.keys(input)) if (!allowed.has(key)) throw new TypeError(`Unknown apply field: ${key}`); for (const field of HANDSHAKE_FIELDS) if (!Object.hasOwn(input, field)) throw new TypeError(`Missing required v3 handshake field: ${field}`); const nested = safeObject(input.handshake, 'handshake'); const nestedAllowed = new Set(['bootstrapTool', 'contractId', 'activationId', 'nonce', 'baseRevision', 'skillProof', 'skillDeliveryProof']); for (const key of Object.keys(nested)) if (!nestedAllowed.has(key)) throw new TypeError(`Unknown handshake field: ${key}`); for (const field of nestedAllowed) if (!Object.hasOwn(nested, field)) throw new TypeError(`Missing handshake field: ${field}`); const context = workspace.getContext?.() || {}; if (!current) throw new TypeError(`Call ${BOOTSTRAP_TOOL} before apply_explanation in this page session`); const result = { requestId: required(input.requestId, 'requestId', 160), expectedWorkspaceRevision: integer(input.expectedWorkspaceRevision, 'expectedWorkspaceRevision'), explanationId: required(input.explanationId, 'explanationId', 160), topicId: topic(input.topicId), bootstrapTool: required(nested.bootstrapTool, 'handshake.bootstrapTool', 128), contractId: required(nested.contractId, 'handshake.contractId', 160), activationId: required(nested.activationId, 'handshake.activationId', 160), nonce: required(nested.nonce, 'handshake.nonce', 160), baseRevision: required(nested.baseRevision, 'handshake.baseRevision', 160), skillProof: verifyProof(nested.skillProof), skillDeliveryProof: verifyDeliveryProof(nested.skillDeliveryProof, current.skillDeliveryProof) }; if (result.bootstrapTool !== BOOTSTRAP_TOOL) throw new TypeError(`handshake.bootstrapTool must be ${BOOTSTRAP_TOOL}; older bootstrap identities are not accepted`); if (result.contractId !== current.contractId || result.activationId !== current.activationId || result.nonce !== current.nonce) throw new TypeError('Activation handshake is stale or does not belong to this page session'); if (result.explanationId !== context.explanationId || result.baseRevision !== context.baseRevision) throw new TypeError('Activation handshake does not match this explanation identity'); return result; }
...
function applySchemaV3() {
  ...
  const deliveryProofItem = { type: 'object', additionalProperties: false, required: ['mode', 'deliveryId', 'registrationId', 'compositeSha256', 'sourceCommit', 'sourceProof'], properties: { mode: { enum: ['native-inline', 'pinned-remote-fallback'] }, deliveryId: string(), registrationId: { type: ['string', 'null'], minLength: 1, maxLength: 160 }, compositeSha256: { type: 'string', pattern: '^[0-9a-f]{64}$' }, sourceCommit: { type: 'string', pattern: '^[0-9a-f]{40}$' }, sourceProof: { type: 'array', minItems: 2, maxItems: 2, items: proofItem } } };
  ...
  return [
    {
      ...
      execute: async (_input = {}, executionOptions = {}) => {
        ...
        throwIfAborted(executionOptions.signal);
        current ||= { ...activation(), nativeRegistrationState: deliveryState.state, skillDeliveryProof: deliveryProof(deliveryState) };
...
export function registerWebMcpTools(workspaceOrPromise, modelContext = null, options = {}) {
  const resolved = modelContext && typeof modelContext.registerTool === 'function'
    ? { modelContext, source: options.hostSource || 'explicit', standard: options.standardHost ?? options.hostSource === 'document.modelContext' }
  ...
  const status = {
    ...
    hostSource: resolved.source, standardHost: resolved.standard,
    expectedTools: [...EXPLAIN_HIM_WEBMCP_TOOLS], registered: [], verifiedTools: [], errors: [], ready: null,
    skillApiAvailable, skillRegistrationState: skillApiAvailable ? 'pending' : 'unavailable',
    registeredSkill: null, skillRegistrationError: null,
    nativeSkillProposalStatus: 'experimental-open-backlog', nativeSkillDigest: EXPLAIN_HIM_NATIVE_SKILL_DIGEST,
  };
  if (!resolved.modelContext) { status.ready = Promise.resolve(status); return status; }
  status.ready = (async () => {
    for (const tool of createWebMcpTools(workspaceOrPromise, { onLifecycle: options.onLifecycle, deliveryState })) {
    ...
    if (skillApiAvailable && status.ok) {
      try {
        ...
        deliveryState.mode = 'native-inline';
        deliveryState.state = 'registered';
        deliveryState.registrationId = opaque('skill-registration');
        ...
        deliveryState.state = 'error';
        status.skillRegistrationState = 'error';
        status.skillRegistrationError = 'registration-failed';
      }
    } else if (skillApiAvailable) {
      deliveryState.state = 'blocked-tools';
    ...
    if (typeof resolved.modelContext.getTools === 'function') {
      try {
        ...
        const names = Array.isArray(available) ? available.map((item) => typeof item === 'string' ? item : item?.name) : [];
        status.verifiedTools = status.expectedTools.filter((name) => names.includes(name));
        status.verified = status.verifiedTools.length === status.expectedTools.length;
