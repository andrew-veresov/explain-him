...
function handshake(input, workspace, current) { if (!input || typeof input !== 'object' || Array.isArray(input)) throw new TypeError('input must be an object'); const allowed = new Set(HANDSHAKE_FIELDS); for (const key of Object.keys(input)) if (!allowed.has(key)) throw new TypeError(`Unknown apply field: ${key}`); for (const field of HANDSHAKE_FIELDS) if (!Object.hasOwn(input, field)) throw new TypeError(`Missing required v3 handshake field: ${field}`); const nested = safeObject(input.handshake, 'handshake'); const nestedAllowed = new Set(['contractId', 'activationId', 'nonce', 'baseRevision', 'skillProof']); for (const key of Object.keys(nested)) if (!nestedAllowed.has(key)) throw new TypeError(`Unknown handshake field: ${key}`); for (const field of nestedAllowed) if (!Object.hasOwn(nested, field)) throw new TypeError(`Missing handshake field: ${field}`); const context = workspace.getContext?.() || {}; const result = { requestId: required(input.requestId, 'requestId', 160), expectedWorkspaceRevision: integer(input.expectedWorkspaceRevision, 'expectedWorkspaceRevision'), explanationId: required(input.explanationId, 'explanationId', 160), topicId: topic(input.topicId), contractId: required(nested.contractId, 'handshake.contractId', 160), activationId: required(nested.activationId, 'handshake.activationId', 160), nonce: required(nested.nonce, 'handshake.nonce', 160), baseRevision: required(nested.baseRevision, 'handshake.baseRevision', 160), skillProof: verifyProof(nested.skillProof) }; if (!current) throw new TypeError('Call get_explanation_contract before apply_explanation in this page session'); if (result.contractId !== current.contractId || result.activationId !== current.activationId || result.nonce !== current.nonce) throw new TypeError('Activation handshake is stale or does not belong to this page session'); if (result.explanationId !== context.explanationId || result.baseRevision !== context.baseRevision) throw new TypeError('Activation handshake does not match this explanation identity'); return result; }
...
function fingerprint(data, input) { return stable({ requestId: data.requestId, explanationId: data.explanationId, topicId: data.topicId, contractId: data.contractId, activationId: data.activationId, nonce: data.nonce, baseRevision: data.baseRevision, skillProof: data.skillProof, operations: input.operations }); }
