...
class BrowserE2E(unittest.TestCase):
    ...
    def test_p0_user_consumer_replace_update_toggle_undo_and_reload(self) -> None:
        ...
        self.assertEqual(page.locator("#webmcp-agent-status").inner_text(), "Agent connection – observed")
        self.assertEqual(page.locator("#webmcp-contract-status").inner_text(), "Contract – activated")
        ...
        self.assertEqual(page.locator("#webmcp-revision-status").inner_text(), "Personalized UI updated – workspace revision 2")
        ...
        self.assertEqual(page.locator("#webmcp-revision-status").inner_text(), "Personalized UI update failed – workspace revision 2")
    ...
    def test_hostless_chrome_fallback_is_honest_and_accessible(self) -> None:
        page, errors = self.page(False); status = page.locator("#webmcp-status-hero").inner_text(); self.assertIn("unavailable", status); self.assertIn("accessible browser controls remain available", status)
        self.assertEqual(page.locator("#webmcp-page-status").inner_text(), "Page WebMCP API – unavailable")
        self.assertEqual(page.locator("#webmcp-agent-status").inner_text(), "Agent connection – not observed")
