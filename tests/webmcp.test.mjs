...
import { EXPLAIN_HIM_UI_TOOLS, createExplainHimSkillDescriptor, registerWebMcpTools } from '../runtime/webmcp.mjs';
...
test('descriptor contains presentation capabilities but no knowledge bundle', () => {
...
test('all UI tools register before native skill', async () => {
  const events = [];
  const host = { registerTool: async (tool) => { events.push(`tool:${tool.name}`); }, registerSkill: async (skill) => { events.push(`skill:${skill.name}`); } };
  const status = registerWebMcpTools(fakeWorkspace(), host); await status.ready;
  ...
  assert.ok(skillIndex > -1); assert.ok(events.slice(0, skillIndex).every((event) => event.startsWith('tool:')));
});

test('compatibility tool is used without registerSkill', async () => {
  const registered = []; const host = { registerTool: async (tool) => registered.push(tool.name) };
  const status = registerWebMcpTools(fakeWorkspace(), host); await status.ready;
  assert.equal(status.skill.mode, 'compatibility-tool'); assert.equal(status.skill.compatibilityToolRegistered, true);
